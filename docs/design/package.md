...
All packages live under `internal/`. The module name is `elichika`, so full import paths are
`elichika/internal/<package>`.
...
> **Note:** Dictionary functionality is not a standalone package. It is integrated into `gamedata`, `serverdata`, and
`internal/generic` (`dictionary.go`).
...
> **Note:** There is no dedicated `middleware` package. Common request processing (authentication, session setup, etc.)
> is handled in `internal/server/router.go` and shared utilities in `internal/handler/common`.
