...
func init() {
	...
	config.MasterVersionGl = readMasterdataManifest(config.GlMasterdataPath + "masterdata_a_en")
	config.MasterVersionJp = readMasterdataManifest(config.JpMasterdataPath + "masterdata_a_ja")

	log.Println("gl master version:", config.MasterVersionGl)
	log.Println("jp master version:", config.MasterVersionJp)
}

func readMasterdataManifest(path string) string {
