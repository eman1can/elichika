...
func init() {
	server.AddSpecialSetup("/webui/user", func(group *gin.RouterGroup) {
		group.StaticFile("/login", config.RootPath+"internal/webui/user/login.html")
	})
	server.AddHandler("/webui/user", "POST", "/login", Login)
}
