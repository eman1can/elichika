...
func init() {
	...
	server.AddHandler("/webui/user", "GET", "/transfer", transferForm)
	server.AddHandler("/webui/user", "POST", "/export_json", exportJson)
	server.AddHandler("/webui/user", "POST", "/export_db", exportDb)
	server.AddHandler("/webui/user", "POST", "/transfer", transferAccount)
}
