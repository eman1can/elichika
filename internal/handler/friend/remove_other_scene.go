...
func removeOtherScene(ctx *gin.Context) {
	req := request.RemoveFriendRequest{}
	err := json.Unmarshal(*ctx.MustGet("reqBody").(*json.RawMessage), &req)
	utils.CheckErr(err)

	session := ctx.MustGet("session").(*userdata.Session)
	successResponse, failureResponse := user_social.RemoveFriendOtherScene(session, req.UserId)
...
func init() {
	server.AddHandler("/", "POST", "/friend/removeOtherScene", removeOtherScene)
}
