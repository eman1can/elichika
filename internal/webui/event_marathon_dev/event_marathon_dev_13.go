...
import (
	...
	"elichika/internal/webui/form/image_form"
...
func init() {
	if config.DeveloperMode == config.DeveloperModeEventMarathonDev {
		server.AddHandler("/webui/event_marathon_dev", "GET", "/13", EventMarathonDev13GET)
		server.AddHandler("/webui/event_marathon_dev", "POST", "/13", EventMarathonDev13POST)
	}
}
