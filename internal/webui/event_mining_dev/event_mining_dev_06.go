...
import (
	...
	"elichika/internal/webui/form/select_form"
...
func init() {
	if config.DeveloperMode == config.DeveloperModeEventMiningDev {
		server.AddHandler("/webui/event_mining_dev", "GET", "/06", EventMiningDev06GET)
		server.AddHandler("/webui/event_mining_dev", "POST", "/06", EventMiningDev06POST)
	}
}
