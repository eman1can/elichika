...
func init() {
	server.AddSpecialSetup("/webui/admin", func(group *gin.RouterGroup) {
		group.StaticFile("/login", config.RootPath+"internal/webui/admin/login.html")
	})
	server.AddHandler("/webui/admin", "POST", "/login", login)
}
