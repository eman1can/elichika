...
import (
	...
	"elichika/internal/webui/form/object_form"
...
func init() {
	server.AddHandler("/webui/admin", "GET", "/event_selector", EventSelector)
	server.AddHandler("/webui/admin", "POST", "/change_event", ChangeEvent)
	server.AddHandler("/webui/admin", "GET", "/event_scheduler", EventScheduler)
	server.AddHandler("/webui/admin", "POST", "/schedule_event", ScheduleEvent)
}
