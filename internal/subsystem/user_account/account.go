...
import (
	...
	"elichika/internal/subsystem/user_live_party"
...
func CreateNewAccount(ctx *gin.Context, userId int32, passWord string) int32 {
	...
	data := ctx.MustGet("gamedata").(*gamedata.Gamedata)
	...
	{ // show formation
		...
		for a := 0; a <= 10000; a += 10000 { // 10000... are DLP formations
			for b := 1; b <= 20; b++ {
				...
				for j := 1; j <= 9; j++ {
					cid[j] = generic.NewNullable(data.Member[int32(j+100*((i-1)%3))].MemberInit.SuitMasterId)
				}
				...
				for j := 1; j <= 3; j++ {
					...
					liveParty.IconMasterId, liveParty.Name.DotUnderText = data.
	...
	{ // lesson deck
		...
		for j := 1; j <= 9; j++ {
			cid[j] = generic.NewNullable(data.Member[int32(j)].MemberInit.SuitMasterId)
		}
		...
		user_lesson_deck.InsertLessonDecks(session, lessonDecks)
	}
	session.Finalize()
